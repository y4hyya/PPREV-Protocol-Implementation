...
The on-chain gas figures published in the paper (Section IV.E) are regenerated by a single command:

```bash
./scripts/run_and_verify.sh
```

This runs `forge test --gas-report --json`, parses the per-function gas, writes the analysis tables to `output/analysis_tables.md`, and compares each measurement against the values committed in `scripts/baseline.json`. The script exits non-zero if any cell drifts beyond the configured tolerance (1% by default), making it a reliable artifact-evaluation gate.

**Prerequisites:** Foundry (the script will use `~/.foundry/bin/forge` if `forge` is not on `$PATH`) and `jq`.

**Output (regenerated each run, gitignored):**

| Path | Contents |
|------|----------|
| `output/analysis_tables.md` | Tables 1–4 in the format used in the paper, with drift vs baseline per row |
| `output/gas_raw.json` | Raw `forge` gas-report JSON for diffing |
| `output/forge.stderr` | Forge stderr if anything failed |

**Updating the baseline.** If a contract change is intentional and the new gas figures are the published values for the next paper revision, edit `scripts/baseline.json` and re-run.
